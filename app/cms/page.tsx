      ...
      <div className={styles.header} style={{
        ...
        {editingLp ? (
          <div className={styles.flexGap}>
             ...
             <button onClick={() => { if (editingLp?.customDomain && editingLp?.slug) { window.open(`https://${editingLp.customDomain}/${editingLp.slug}?preview=true`, '_blank'); } else if (editingLp?.slug) { window.open(`/${editingLp.slug}?preview=true`, '_blank'); } }} disabled={!editingLp?.slug} className={`${styles.btn} ${styles.btnSecondary}`} style={{display:'flex', alignItems:'center', gap:4}}>
