      ...
      <div className={styles.header} style={{
        ...
        {editingLp ? (
          <div className={styles.flexGap}>
             ...
             <button onClick={() => { const q = editingLp?.status === 'public' ? '' : '?preview=true'; if (editingLp?.customDomain && editingLp?.slug) { window.open(`https://${editingLp.customDomain}/${editingLp.slug}${q}`, '_blank'); } else if (editingLp?.slug) { window.open(`/${editingLp.slug}${q}`, '_blank'); } }} disabled={!editingLp?.slug} className={`${styles.btn} ${styles.btnSecondary}`} style={{display:'flex', alignItems:'center', gap:4}}>
